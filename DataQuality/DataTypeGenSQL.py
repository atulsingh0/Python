CONF = "ConfigDataType.txt"
DataTypeDetailQuery = 'DataType_Detail_Query.sql'
# NullInsertQuery = 'Null_Insert_Query.sql'
# NullHiveQuery = 'Null_Hive_Query.sql'

conf = open(CONF, 'r');
DataType_detail_sqls=[]
# null_insert_sqls=[]
# null_hive_sqls=[]
...
for line in conf:
	...
	# select 'DQ_FMT_AMC_BROKER_ACCOUNT_1','AMC_BROKER_ACCOUNT.ETL_FILEID ,AMC_BROKER_ACCOUNT.ETL_BATCHID ,AMC_BROKER_ACCOUNT.EAP_AS_OF_DT ,AMC_BROKER_ACCOUNT.IMSNUMBER' pknames ,AMC_BROKER_ACCOUNT.ETL_FILEID pk1,AMC_BROKER_ACCOUNT.ETL_BATCHID PK2,AMC_BROKER_ACCOUNT.EAP_AS_OF_DT pk3,AMC_BROKER_ACCOUNT.IMSNUMBER pk4,'-' pk5,'-' pk6,'-' pk7,'-' pk8,AMC_BROKER_ACCOUNT.OPENDATE errcol   from AMC_BROKER_ACCOUNT where Not(AMC_BROKER_ACCOUNT.OPENDATE is   null or AMC_BROKER_ACCOUNT.OPENDATE = '') and 
    # (OPENDATE rlike   '^(19|20)\\d\\d-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])T([01]?[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])\\.\\d\\d\\d\\+00:00$' )


 
    # generating DATA TYPE CHK Detailed Query
    DataType_detail_query = "select '{chk_id}', '{pknames}' pknames, {pk1} pk1, {pk2} pk2, {pk3} pk3, {pk4} pk4, {pk5} pk5,{pk6} pk6, {pk7} pk7, {pk8} pk8,{errcol} errcol from {table} where Not ({table}.{SrcCol} is null or {table}.{SrcCol} = '') and ({SrcCol} rlike {FtrRule})"\
                  .format(chk_id=ChkId, pknames=pknames, pk1=pk1, pk2=pk2, pk3=pk3, pk4=pk4, pk5=pk5, pk6=pk6, pk7=pk7, pk8=pk8, errcol=errcol, table=SrcTab,SrcCol=SrcCol,FtrRule=FtrRule );
    DataType_detail_query1 = DataType_detail_query+"\n";
    DataType_detail_sqls.append(DataType_detail_query1)
...
DataTypedetail = open(DataTypeDetailQuery, 'w')
DataTypedetail.writelines(DataType_detail_sqls);
DataTypedetail.close()
