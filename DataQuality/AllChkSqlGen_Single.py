...
for line in conf:
    ...
    if (NullChk=='Y'):
        ...
        NullChkCase='case ('+errcol+' is null and '+errcol+' = '') When True Then 1 Else 0 end'
        null_detail_query = "select '{chk_id}', '{pknames}' pknames, {pk1} pk1, {pk2} pk2, {pk3} pk3, {pk4} pk4, {pk5} pk5,{pk6} pk6, {pk7} pk7, {pk8} pk8, {errcol} errcol,  {NullChkCase} NullChkResult from {table} where {Null_fil_cond}"\
                  .format(chk_id=ChkId, pknames=pknames, pk1=pk1, pk2=pk2, pk3=pk3, pk4=pk4, pk5=pk5, pk6=pk6, pk7=pk7, pk8=pk8, errcol=errcol, table=SrcTab, Null_fil_cond=Null_fil_cond, NullChkCase=NullChkCase);
